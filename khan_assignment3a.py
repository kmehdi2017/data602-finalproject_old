...
def plotdata(currency):
    ...
    mkt_hist.plot(x=mkt_hist['time'], y= ['high','low'],ax=ax, style=['r','k--'], grid=True, sharex=True, figsize=(10,8), title="100 days high and low")
    mkt_hist.plot(x=mkt_hist['time'], y= ['open','close'],ax=ax2, style=['b','g--'], grid=True, sharex=True,title="100 days open and close" )
    mkt_hist.plot.area(x=mkt_hist['time'], y=['moving average'],ax=ax3, style=['b'], alpha=0.25, grid=True, title="20 days moving average" )
...
def plot_PL_timeSeries():
    ...
    for i, name in enumerate(unique_ticker):
        ...
        df_subset.plot(x=df_subset['Time'], y= ['WAP'],ax=axes[i], style=['r'], grid=True,  title="WAP for: "+ name,rot=15)
...
def plotPLgraphs(df,yval,stl,title):
    ...
    df.plot(x=df['Time'], y= yval,ax=ax, style=stl, grid=True,figsize=(8,3), title=title,rot=35)
    img = io.BytesIO()
    plt.savefig (img, format='png')
    img.seek(0)    

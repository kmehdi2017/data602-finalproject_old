...
def predict():
    ...
    for m in currencies:
        full_market_info = pd.read_html("https://coinmarketcap.com/currencies/"+m+"/historical-data/?start="+start_date+"&end="+time.strftime("%Y%m%d"))[0]
    ...
    for n in currencies:
        pre_market_info = pd.read_html("https://coinmarketcap.com/currencies/"+n+"/historical-data/?start=20150101&end="+spike_date)[0]
        if (pre_market_info[['Date']]=='No data was found for the selected time period.')== True:
            pass
        else:
